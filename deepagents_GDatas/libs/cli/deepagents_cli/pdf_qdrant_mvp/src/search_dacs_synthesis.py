...
3. **双原子催化剂活性位点：**
   ...
   - 示例：`{"active_site": "Fe2N6", "loading": "0.1 mg/cm²", "metal_metal_distance": "2.22 Å"}`
...
```json
{
  ...
  "double_atom_catalyst_active_site": {
    ...
    "metal_metal_distance": "2.22 Å"
  }
}
"""


def extract_atomic_distance(text):
    """
    从文本中提取原子间距离（单位 Å）
    返回列表，每个元素为距离数值
    """
    # 匹配模式：数字（整数或小数） + 可选空格 + Å 或 A 或 angstrom
    pattern = r'(\d+\.?\d*)\s*[ÅA](?:\s*angstroms?)?'
...
def search_catalyst_synthesis_in_all_collections():
        ...
        try:
            ...
            if results:
                for r in results:
                    ...
                    # 提取并显示原子间距
                    distances = extract_atomic_distance(text)
                    if distances:
                        print(f"  🔍 检测到原子间距 (Å): {', '.join(map(str, distances))}")
                    else:
                        print(f"  🔍 未检测到原子间距")
                    print()
            else:
    ...
    for i, r in enumerate(all_results[:10]):
        ...
        print(f"内容:\n{r['text']}")
        # 提取并显示原子间距
        distances = extract_atomic_distance(r['text'])
        if distances:
            print(f"🔍 原子间距 (Å): {', '.join(map(str, distances))}")
        else:
            print(f"🔍 未检测到原子间距")
        print("-" * 40)
